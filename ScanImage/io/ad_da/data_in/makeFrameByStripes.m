...
if state.init.eom.pockelsOn && state.init.eom.powerVsZEnable  
    if state.motor.motorZEnable && state.motor.dimensionsXYZZ
        state.init.eom.stackPowerScaling = exp(state.init.eom.powerVsZEnable .* (state.motor.absZZPosition - state.internal.initialMotorPosition(4))./((-1)^state.motor.zDepthPositive * -1 * state.init.eom.powerLzArray)); %VI010610A
    else        
        state.init.eom.stackPowerScaling = exp(state.init.eom.powerVsZEnable .* (state.motor.absZPosition - state.internal.initialMotorPosition(3))./((-1)^state.motor.zDepthPositive * -1 * state.init.eom.powerLzArray)); %VI010610A
    end
end	
